...
def train_conditional_generator_supervised_streamlit(
    ...
    final_mae = float(np.mean(np.abs(pred_scaled - y_scaled.reshape(-1))))
    status_text.text(f"Training completed. Final scaled MAE: {final_mae:.8f}")

    return generator, pd.DataFrame(history_rows), final_mae
...
class DecisionTreeRegressorRaw:
    class Node:
    ...
    def _best_split(self, X, y):
        ...
        for j in range(d):
            ...
            for t in thresholds:
                ...
                loss = (L.sum() / n * self._mse(y[L])) + (R.sum() / n * self._mse(y[R]))
                gain = parent_loss - loss

                if gain > best_gain:
                    best_gain, best_f, best_t = gain, j, t
    ...
    def _build(self, X, y, depth):
        ...
        return self.Node(f, t, left, right)
...
class XGBoostRegressorRaw:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            ...
            if self.subsample < 1:
                m = int(n * self.subsample)
                idx = rng.choice(n, m, replace=False)
                Xb, rb = X[idx], residuals[idx]
            else:
                Xb, rb = X, residuals

            tree = DecisionTreeRegressorRaw(
...
def build_xgb_training_frames(real_df, synthetic_df):
...
def build_scenario_table(original_df, prediction_df):
    ...
    pred_one = (
        ...
        .mean()
        .rename(columns={"Vertical Force": "Vertical Force XGBoost_WGAN"})
    )
    ...
    scenario_table_36 = scenario_table_36[
        [
            ...
            "Vertical Force original", "Vertical Force XGBoost_WGAN", "Experimental"
        ]
    ]

    scenario_table_36["Vertical Force XGBoost_WGAN"] = scenario_table_36["Vertical Force XGBoost_WGAN"].astype(float).round(9)
...
defaults = {
    ...
    "balanced_df": None,
    "xgb_prediction_df": None,
    "scenario_table": None,
    "distribution_metrics": None,
    "prediction_metrics": None,
    "final_scaled_mae": None,
}
...
final_fit_epochs = st.sidebar.slider("Final Zero-Noise Fit Epochs", min_value=50, max_value=1000, value=500, step=50)
...
st.sidebar.subheader("XGBoost-like Settings")
n_estimators = st.sidebar.slider("Estimators", min_value=10, max_value=500, value=300, step=10)
xgb_learning_rate = st.sidebar.slider("Boosting Learning Rate", min_value=0.01, max_value=0.3, value=0.05, step=0.01)
max_depth = st.sidebar.slider("Tree Max Depth", min_value=1, max_value=10, value=3, step=1)
min_samples_split = st.sidebar.slider("Min Samples Split", min_value=2, max_value=30, value=10, step=1)
subsample = st.sidebar.slider("Subsample", min_value=0.1, max_value=1.0, value=0.8, step=0.1)

# main gui
st.title("WGAN + XGBoost-like Prediction")
st.write("Upload `WindTunnelData.csv`, train the WGAN-style supervised generator, generate synthetic data, and evaluate the custom XGBoost-like regressor.")
    ...
    try:
        ...
        if missing_cols:
            ...
            if st.button("Start Training"):
                with st.spinner("Training WGAN-style supervised generator..."):
                    generator, history_df, final_scaled_mae = train_conditional_generator_supervised_streamlit(
                        df_raw=df1,
                    ...
                    st.session_state.final_scaled_mae = final_scaled_mae
            ...
            if st.session_state.training_history is not None:
                ...
                if st.session_state.final_scaled_mae is not None:
                    st.metric("Final Scaled MAE", f"{st.session_state.final_scaled_mae:.8f}")

            st.subheader("Generate Synthetic Data and Run XGBoost-like Prediction")

            if st.session_state.generator is not None:
                if st.button("Generate Synthetic Data + Run XGBoost-like"):
                    with st.spinner("Generating synthetic data and evaluating XGBoost-like model..."):
                        ...
                        w1 = wasserstein_distance(y_real_dist.squeeze(), y_hat_dist.squeeze())

                        X_real, y_real, X_syn, y_syn = build_xgb_training_frames(
                            st.session_state.clean_df,
                            balanced
                        )

                        xgb_model = XGBoostRegressorRaw(
                        ...
                        xgb_prediction_df = st.session_state.clean_df[
                            ["Flap deflection", "ANGLE OF ATTACK", "Force Quantities"]
                        ].copy()
                        xgb_prediction_df["Vertical Force"] = y_pred_real

                        scenario_table = build_scenario_table(st.session_state.clean_df, xgb_prediction_df)

                        st.session_state.balanced_df = balanced
                        st.session_state.xgb_prediction_df = xgb_prediction_df
            ...
            if st.session_state.xgb_prediction_df is not None:
                st.subheader("XGBoost-like Predictions Preview")
                st.dataframe(st.session_state.xgb_prediction_df.head(50), use_container_width=True)
                st.download_button(
                    label="Download XGBoost-like Prediction CSV",
                    data=dataframe_to_csv_bytes(st.session_state.xgb_prediction_df),
                    file_name="WGAN_XGBoost_Predictions.csv",
                    mime="text/csv"
                )
            ...
            if st.session_state.scenario_table is not None:
                ...
                st.download_button(
                    ...
                    file_name="XGBoost_on_WGAN_Table.csv",
                    mime="text/csv"
                )
