...
st.set_page_config(page_title="WGAN + SVR Prediction", layout="wide")
...
def train_conditional_generator_supervised_streamlit(
    ...
    zero_noise = np.zeros((n, latent_dim), dtype=np.float32)
    generator.fit(
...
def build_svr_outputs(synthetic_df, original_df, C, epsilon, lr, epochs, seed):
    cat_cols = ["Flap deflection", "ANGLE OF ATTACK", "Force Quantities"]
    target_col = "Vertical Force"

    train_enc = pd.get_dummies(synthetic_df[cat_cols], columns=cat_cols, drop_first=True)
    X_cols = list(train_enc.columns)
    X_syn = train_enc.to_numpy(np.float32)
    y_syn = synthetic_df[target_col].to_numpy(np.float32)

    mu, sigma = standardize_fit(X_syn)
    X_syn_s = standardize_transform(X_syn, mu, sigma)

    svr = LinearSVRRaw(C=C, epsilon=epsilon, lr=lr, epochs=epochs, random_state=seed)
    svr.fit(X_syn_s, y_syn)
    ...
    wanted_enc = pd.get_dummies(wanted[cat_cols], columns=cat_cols, drop_first=True)
    for c in X_cols:
    ...
    svr_pred = svr.predict(X_wanted_s).astype(np.float32)

    svr_prediction_df = wanted[cat_cols].copy()
    svr_prediction_df["Vertical Force"] = svr_pred

    svr_one = wanted[["Category"]].copy()
    svr_one["Vertical Force SVR_WGAN"] = svr_pred
    ...
    scenario_table_36 = scenario_table_36[
        ["Flap deflection", "ANGLE OF ATTACK", "Force Quantities",
         "Vertical Force original", "Vertical Force SVR_WGAN", "Experimental"]
    ]

    metric_df = scenario_table_36.dropna(subset=["Vertical Force original", "Vertical Force SVR_WGAN"]).copy()
    mae = mean_absolute_error(metric_df["Vertical Force original"], metric_df["Vertical Force SVR_WGAN"])
    rmse = np.sqrt(mean_squared_error(metric_df["Vertical Force original"], metric_df["Vertical Force SVR_WGAN"]))
    r2 = r2_score(metric_df["Vertical Force original"], metric_df["Vertical Force SVR_WGAN"])

    return svr_prediction_df, scenario_table_36, {"MAE": float(mae), "RMSE": float(rmse), "R²": float(r2)}
...
noise_std = st.sidebar.slider("Generator Noise Std", min_value=0.0, max_value=0.2, value=0.05, step=0.01)
...
st.sidebar.subheader("Linear SVR Settings")
...
svr_lr = st.sidebar.selectbox("SVR Learning Rate", [1e-4, 1e-3, 1e-2, 5e-2], index=2)
svr_epochs = st.sidebar.slider("SVR Epochs", min_value=100, max_value=5000, value=1500, step=100)


# main GUI
st.title("WGAN + SVR Prediction")
st.write("Upload `WindTunnelData.csv`, train the WGAN-style generator, generate synthetic data, and evaluate custom SVR prediction.")
...
if uploaded_file is not None:
    try:
        ...
        if missing_cols:
            ...
            with col3:
                st.metric("Condition Columns", len(COND_COLS))

            st.subheader("Train WGAN-Style Generator")

            if st.button("Start Training"):
                with st.spinner("Training WGAN-style generator..."):
            ...
            st.subheader("Generate Synthetic Data and Run SVR")
            ...
            if st.session_state.generator is not None:
                if st.button("Generate Synthetic Data + Run SVR"):
                    with st.spinner("Generating synthetic data and evaluating SVR..."):
                        ...
                        svr_prediction_df, scenario_table, pred_metrics = build_svr_outputs(
                            synthetic_df=balanced,
                            original_df=st.session_state.clean_df,
                            C=svr_c,
                            epsilon=svr_epsilon,
                            lr=svr_lr,
                            epochs=svr_epochs,
                        ...
                        st.session_state.prediction_metrics = pred_metrics
            ...
            if st.session_state.svr_prediction_df is not None:
                ...
                st.download_button(
                    ...
                    file_name="WGAN_SVR_Predictions.csv",
                    mime="text/csv",
                )
